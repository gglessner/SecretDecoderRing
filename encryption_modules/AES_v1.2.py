...
def decrypt(iv, key, ciphertext):
    ...
    def try_decrypt(mode_name, mode, mode_iv, ct, extra_label=""):
        try:
            if mode == AES.MODE_ECB:
                ...
                if mode_iv is None or len(mode_iv) not in (8, 12, 16):
                    return
                ...
                if mode_iv is None or len(ct) < 16:
                    return
                tag = ct[-16:]
                actual_ct = ct[:-16]
    ...
    modes = [
        ...
        ('CTR', AES.MODE_CTR, iv if iv and len(iv) in (8, 12, 16) else None),
    ...
    # Try plain ECB mode explicitly, like the working code
    if len(ciphertext) % 16 == 0 and len(ciphertext) >= 16:
        try:
            ...
            if 1 <= pad_len <= 16 and pt_padded[-pad_len:] == bytes([pad_len]) * pad_len:
                pt = pt_padded[:-pad_len]
                if try_utf8(pt):
                    results.append(("ECB (explicit)", pt))
        except Exception:
            pass
